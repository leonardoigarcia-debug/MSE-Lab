...
uint32_t vector_tbl[] __attribute__((section(".isr_vector_tbl"))) = {
  ...
  (uint32_t)&_estack,                /* Top of the Stack, Initial value of the MSP            */
...
void Reset_Handler(void){
    ...
    /* Convert byte sizes to 32-bit word counts to match uint32_t
       copies (4 == sizeof(uint32_t)) */
    data_mem_size /= 4;
    bss_mem_size /= 4;

    /* Initialize pointers to the source and destination of the
       .data section */
